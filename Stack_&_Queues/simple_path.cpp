...
public:
    string simplifyPath(string path)
    {
        ...
        { // getline splits the string by '/' , it takes three arguments: the input stream, the string to store the result, and
            ...
            if (dir.empty() || dir == ".")
            { // If the directory is empty or is a single dot, we skip it.
                continue;
            }
            else if (dir == "..")
            { // If the directory is a double dot, we pop the top element from the stack if it is not empty.
                if (!s.empty())
                {
                    s.pop();
                }
            ...
            { // If the directory is a valid name, we push it onto the stack.
                s.push(dir);
            }
        }
        string res;
        while (!s.empty())
        {                              // We construct the result by popping elements from the stack.
